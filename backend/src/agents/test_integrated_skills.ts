...
function runIntegratedSkillsTests() {
  ...
  const spec1 = controller.compileAgent({ prompt: prompt1 });
  ...
  const spec2 = controller.compileAgent({ prompt: prompt2 });
  ...
  const spec3 = controller.compileAgent({ prompt: prompt3 });
